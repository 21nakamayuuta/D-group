...
public class PgRecipeDao implements RecipeDao {
	...
	private static final String SELECT_NEW_RECIPE = "select r.recipe_id,r.recipe_title,r.complete_image,coalesce(count(g.good_id),0) as goodCount,r.create_datetime from recipe r left join good_table g on r.recipe_id = g.recipe_id GROUP BY r.recipe_id order by r.create_datetime desc OFFSET 0 LIMIT 6";
	...
	private static final String SELECT_RANKING2 = "select g.rank rank, g.c good_count, r.recipe_id,r.recipe_title,r.complete_image,r.create_datetime "
			+ "from recipe r join "
			+ "(select rank() over(order by count(user_id) desc), recipe_id, count(user_id) c from good_table group by recipe_id) g "
			+ "on r.recipe_id = g.recipe_id order by g.rank limit 3";
	...
	public List<Recipe> ranking() {
		// List<Recipe> rankingList = jT.query(SELECT_RANKING, new
		// BeanPropertyRowMapper<Recipe>(Recipe.class));
		List<Recipe> rankingList = jT.query(SELECT_RANKING2, new BeanPropertyRowMapper<Recipe>(Recipe.class));
		return rankingList;
	}
