...
public class PgRecipeSearchDao implements RecipeDao {
	...
	private static final String SELECT_NEW_RECIPE = "select r.recipe_id,r.recipe_title,r.complete_image,count(g.good_id) as goodCount,r.create_datetime from recipe r join good_table g on r.recipe_id = g.recipe_id GROUP BY r.recipe_id order by r.create_datetime OFFSET 0 LIMIT 6";
